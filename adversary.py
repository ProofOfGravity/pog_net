...
class XOGame:
    ...
    def __init__(self):
        ...
        self.reward_buffer_o = deque(maxlen=100)
    ...
    def play_step(self, action_x, action_o):
        ...
        state_o_initial = self.calculate_state_o()

        if self.frame_iteration % 2 == 0:
            action_o = 4
        ...
        state_o_next = self.calculate_state_o()

        # 4. Add transition to replay buffer
        transition_x = (state_x_initial, action_x, x_reward, game_over, state_x_next)
        self.replay_buffer_x.append(transition_x)

        transition_o = (state_o_initial, action_o, o_reward, game_over, state_o_next)
        self.replay_buffer_o.append(transition_o)
    ...
    def initialize_replay_memory(self):
        ...
        for _ in range(REPLAY_INITIAL_SIZE):
            ...
            # 4. Add transition to replay buffer
            transition = (state_o_initial, x_move, x_reward, game_over, state_o_next)
            self.replay_buffer_o.append(transition)
    ...
    def check_move(self, action_x, action_o):
        ...
        if game_over and winner == 'X':
            ...
            reward_o = reward_o + 1 if dist_curr > dist_prev else reward_o - 1

        reward_x -= 0.1 * (self.frame_iteration % 50)
    ...
    def calculate_state_o(self):
        ...
        state_o = np.zeros(8, dtype=int)

        # Check for danger in all four directions, meaning going off grid or hitting block
        if self.o_pos.y == 0 or Point(self.o_pos.x, self.o_pos.y - 1) in self.blocks:
            state_o[0] = 1
        if self.o_pos.y == GRID_SIZE - 1 or Point(self.o_pos.x, self.o_pos.y + 1) in self.blocks:
            state_o[1] = 1
        if self.o_pos.x == 0 or Point(self.o_pos.x - 1, self.o_pos.y) in self.blocks:
            state_o[2] = 1
        if self.o_pos.x == GRID_SIZE - 1 or Point(self.o_pos.x + 1, self.o_pos.y) in self.blocks:
            state_o[3] = 1
    ...
    def learn_without_display(self, x_net, o_net, x_trainer, o_trainer, iterations):
        for _ in range(iterations):
            ...
            if rnd_sample <= epsilon:
                a_o = random.randint(0, 4)
            else:
                a_o = o_net.act(game.calculate_state_o())
            ...
            # 1. Check if game is over, if so, report Winner/Loser and reset
            game_over, winner, loser, x_reward, o_reward = self.check_move(a_x, a_o)
            self.reward_x_total += x_reward
            ...
            # 4. Add transition to replay buffer
            transition_x = (state_x_initial, a_x, x_reward, game_over, state_x_next)
            self.replay_buffer_x.append(transition_x)

            transition_o = (state_o_initial, a_o, o_reward, game_over, state_o_next)
            self.replay_buffer_o.append(transition_o)
            ...
            o_trainer.train_step(game.replay_buffer_o, 100, GAMMA)
...
o_net = Linear_QNet(8, 64, 5)
o_trainer = QTrainer(x_net, 0.0005, GAMMA)

game.initialize_replay_memory()

game.learn_without_display(x_net, o_net, x_trainer, o_trainer, 10000)
